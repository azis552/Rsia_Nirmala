                        ...
                        <table id="myTable" class="display  nowrap" style="width:100%">
                            ...
                            <tbody>
                                @foreach ($dokters as $dokter)
                                    <tr>
                                        <td>{{ $dokter->name }}</td>
                                        <td><img src="{{ asset("storage/dokter/".$dokter->foto) }}" alt="Gambar dokter"
                                                style="max-width: 100px; height: auto;"></td>
                                        <td>
                                            {{$dokter->poliklinik->name ?? 'Poliklinik tidak ditemukan'}}
                                        </td>
                                        <td>
                                            <form action="{{ Route('dokter.destroy', $dokter->id) }}" method="post">
                                                ...
                                                <button type="button" class="btn btn-warning text-white btn-edit"
                                                    ...
                                                    data-id="{{ $dokter->id }}" data-name="{{ $dokter->name }}"
                                                    data-gambar="{{ $dokter->foto }}" data-poliklinik="{{ $dokter->poliklinik_id }}"
                                                    >
                    ...
                    <div class="modal-dialog modal-lg">
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                <form id="formTambah" action="{{ Route('dokter.store') }}" method="post"
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select name="poliklinik" id="poliklinik" class="form-select" required>
                                            <option value="">Pilih Poliklinik</option>
                                            @foreach ($polikliniks as $poliklinik)
                                                <option value="{{ $poliklinik->id }}">{{ $poliklinik->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                    </div>
                        ...
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                <form id="formEdit" method="post" enctype="multipart/form-data">
                                    ...
                                    <div class="mb-3">
                                        <label for="poliklinik" class="form-label">Poliklinik</label>
                                        <select name="poliklinik" id="poliklinikEdit" class="form-select" required>
                                            <option value="">Pilih Poliklinik</option>
                                            @foreach ($polikliniks as $poliklinik)
                                                <option value="{{ $poliklinik->id }}">{{ $poliklinik->name }}
                                                </option>
    ...
    <script>
        ...
        $(document).on('click', '.btn-edit', function() {
            ...
            var poliklinik = $(this).data('poliklinik');
                $('#idEdit').val(id);
                $('#nameEdit').val(name);
                $('#poliklinikEdit').val(poliklinik);

                $('#previewEdit').attr('src', "{{ asset('storage/dokter/') }}/" +  gambar);
                $('#formEdit').attr('action', '{{ url('dokter') }}/' + id);
        })
