        ...
        <div class="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div class="md:col-span-2 bg-white rounded-2xl shadow-lg p-6 space-y-6">
                <div class="w-full aspect-video overflow-hidden rounded-xl">
                    <img src="{{ asset('storage/fasilitasUnggulan/' . $fasilitasUnggulan->image1) }}" alt="Gambar Artikel"
                        class="w-full h-full object-cover" />
                </div>
                <div class="space-y-2">
                    <span class="inline-block px-3 py-1 bg-green-600 text-white rounded-full text-sm font-medium">Kategori:
                       Fasilitas Unggulan</span>
                    <h1 class="text-3xl font-bold text-green-700">{{ $fasilitasUnggulan->name }}</h1>
                    <hr>
                    <p class="text-gray-700 text-sm">{{ $fasilitasUnggulan->created_at->format('d F Y') }}</p>
                    <p class="text-gray-800 text-base leading-relaxed">
                        {!! $fasilitasUnggulan->description !!}
                    </p>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    @for ($i = 2; $i <= 5; $i++)
                        @php
                            $image = 'image' . $i;
                        @endphp
                        @if (!empty($fasilitasUnggulan->$image))
                            <div
                                class="border border-gray-300 rounded-xl p-3 shadow-md hover:shadow-lg transition duration-300 bg-white">
                                <img src="{{ asset('storage/fasilitasUnggulan/' . $fasilitasUnggulan->$image) }}" alt="Gambar Artikel"
